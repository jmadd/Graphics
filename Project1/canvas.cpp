...
void myKeyHandler(unsigned char ch, int x, int y) {
	switch(ch) {
		...
		case 'I':
			r+=0.1f;
			printf("Radius has increased by 0.1\n");
			break;
		case 'i':
			if(r > 0.1f){
				r-=0.1f;
				printf("Radius has decreased by 0.1\n");
			}
			else
				printf("Radius has remained 0.1\n");
			break;
		case 'O':
			h+=0.1f;
			printf("Height has increased by 0.1\n");
			break;
		case 'o':
			if(h > 0.1f){
				h-=0.1f;
				printf("Height has decreased by 0.1\n");
			}else
				printf("Height has remained 0.1\n");
			break;
		case 'P':
			n++;
			printf("Number of triangles has increased by 1\n");
			break;
		case 'p':
			if(n > 3){
				n--;
				printf("Number of triangles has decreased by 1\n");
			}else
		...
		case 'r':
			curr_rec++;
			curr_rec = (curr_rec > MAX_RECURSION) ? 0 : curr_rec;
			break;
